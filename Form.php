...
class Ajax_Form {
    ...
    public function __construct()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $name    = filter_var($this->secure($_POST['contact-name']), FILTER_SANITIZE_STRING) ?? $this->statusHandler('enter_contact-name');
            $email   = filter_var($this->secure($_POST['contact-email']), FILTER_SANITIZE_EMAIL) ?? $this->statusHandler('enter_contact-email');
            $company = filter_var($this->secure($_POST['contact-company']), FILTER_SANITIZE_STRING) ?? $this->statusHandler('enter_contact-company');
            $phone = filter_var($this->secure($_POST['contact-phone']), FILTER_SANITIZE_STRING) ?? $this->statusHandler('enter_contact-phone');
            $message = filter_var($this->secure($_POST['contact-message']), FILTER_SANITIZE_STRING) ?? $this->statusHandler('enter_contact-message');
